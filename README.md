# Central Bank of Belize Exchange Rates API — central-bank-of-belize-exchange-rate

[![npm version](https://img.shields.io/npm/v/central-bank-of-belize-exchange-rate.svg)](https://www.npmjs.com/package/central-bank-of-belize-exchange-rate)
...
- 🏛️ **Official published rates** — Central Bank of Belize's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Central Bank of Belize itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Central Bank of Belize table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cbbz?source=USD&target=BZD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cbbz').then((x) => x.json());
...
```bash
yarn add central-bank-of-belize-exchange-rate
```

```bash
pnpm add central-bank-of-belize-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/central-bank-of-belize-exchange-rate`](https://www.npmjs.com/package/@allratestoday/central-bank-of-belize-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Central Bank of Belize rate, on the central bank's own date
```
...
```javascript
{
  ...
  rate_date: '2026-09-09',   // Central Bank of Belize's own publication date
  source: 'USD',
  target: 'BZD',
  rate: 2.0175,
  rate_type: 'sell',
...
```js
import { getLatestRates } from 'central-bank-of-belize-exchange-rate';
...
```javascript
{
  ...
  rates: [
    ...
    // … the rest of the published table (4 currencies vs BZD)
  ],
  disclaimer: '…'
}
```
...
```js
import { getRatesForDate } from 'central-bank-of-belize-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'BZD' });
```
...
const series = await getHistory(
  { source: 'USD', target: 'BZD', from: '2026-01-01', to: '2026-09-09' },
  { apiKey: 'art_live_...' }
);
```
...
```javascript
{
  ...
  rates: [
    ...
    { date: '2026-09-09', rate: 2.0175, rate_type: 'sell', derived: false, method: 'published' },
    // …
  ],
  disclaimer: '…'
}
...
Central Bank of Belize currently publishes rates covering **4 currencies** against the BZD (as of the latest table):
...
- Publisher's own page: [Exchange rates](https://www.centralbank.org.bz/home/statistics/rates/exchange-rates) · [www.centralbank.org.bz](https://www.centralbank.org.bz)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Belize rates page](https://allratestoday.com/central-bank-rates-api/cbbz/)
...
- Central Bank of Belize quotes **BZD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "BZD"` means BZD per one US dollar).
...
Loading the official Central Bank of Belize rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cbbz/latest?format=xml&api_key=art_live_..."
```
...
If Central Bank of Belize does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via BZD from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Belize |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'central-bank-of-belize-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('central-bank-of-belize-exchange-rate');

getRate('USD', 'BZD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- Hugging Face: [AllRates/central-bank-exchange-rates](https://huggingface.co/datasets/AllRates/central-bank-exchange-rates) — one CSV per institution (`rates/cbbz.csv`)
- Kaggle: [allratestoday/central-bank-exchange-rates](https://www.kaggle.com/datasets/allratestoday/central-bank-exchange-rates)
- CDN JSON: `https://cdn.jsdelivr.net/gh/AllRates-Today/central-bank-exchange-rates@main/data/cbbz/latest.json`

## 🔗 Links

- [Central Bank of Belize rates page](https://allratestoday.com/central-bank-rates-api/cbbz/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/central-bank-of-belize-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/central-bank-of-belize-exchange-rate)
